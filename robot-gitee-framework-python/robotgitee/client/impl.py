...
class _Client(Client):
    ...
    def create_pull_request(self, org: str, repo: str, title: str,
                            body, head, base: str):
        body = gitee.CreatePullRequestParam(
            title=title, body=body, base=base,
            head=head, prune_source_branch=True,
        )
    ...
    def get_pull_requests(self, org: str, repo: str,
        ...
        if opts is not None:
            param = vars(opts)
            return self.pr_api.get_v5_repos_owner_repo_pulls(
                org, repo, **param)
    ...
    def update_pull_request(self, org: str, repo: str, number: int,
        ...
        return self.pr_api.patch_v5_repos_owner_repo_pulls_number(
            org, repo, number, body)

    def list_collaborators(self, org: str,
                           repo: str) -> List[gitee.ProjectMember]:
        return self.repository_api.get_v5_repos_owner_repo_collaborators(
            org, repo)

    def is_collaborator(self, org: str, repo: str, username: str) -> bool:
        try:
            self.repository_api.get_v5_repos_owner_repo_collaborators_username(
                org, repo, username)
        except Exception:
            return False
        return True
    ...
    def is_member(self, org: str, username: str) -> bool:
        try:
            self.organization_api.get_v5_orgs_org_memberships_username(
                org, username)
        except Exception:
            return False
        return True
    ...
    def remove_repo_member(self, org: str, repo: str, username: str):
        api = self.repository_api
        return api.delete_v5_repos_owner_repo_collaborators_username(
            org, repo, username)
    ...
    def add_repo_member(self, org: str, repo: str,
        ...
        self.repository_api.put_v5_repos_owner_repo_collaborators_username(
            org, repo, username, body)

    def get_ref(self, org: str, repo: str, ref: str) -> str:
        branch = self.repository_api.get_v5_repos_owner_repo_branches_branch(
            org, repo, ref)
        return branch.commit.sha

    def get_pull_request_changes(self, org: str, repo: str,
                                 number: int) -> List[gitee.PullRequestFiles]:
        return self.pr_api.get_v5_repos_owner_repo_pulls_number_files(
            org, repo, number)

    def get_pr_labels(self, org: str, repo: str,
                      number: int) -> List[gitee.Label]:
        return self.pr_api.get_v5_repos_owner_repo_pulls_number_labels(
            org, repo, number)

    def list_pr_comments(self, org: str, repo: str,
                         number: int) -> List[gitee.PullRequestComments]:
        comments = self.pr_api.get_v5_repos_owner_repo_pulls_number_comments(
            org, repo, number)
        return comments

    def list_pr_issues(self, org: str, repo: str,
                       number: int) -> List[gitee.Issue]:
        return self.pr_api.get_v5_repos_owner_repo_pulls_number_issues(
            org, repo, number)
    ...
    def create_pr_comment(self, org: str, repo: str, number: int,
        ...
        return self.pr_api.post_v5_repos_owner_repo_pulls_number_comments(
            org, repo, number, body)

    def update_pr_comment(self, org: str, repo: str, comment_id: int,
                          comment: str):
        body = gitee.PullRequestCommentPatchParam(body=comment)
        return self.pr_api.patch_v5_repos_owner_repo_pulls_comments_id(
            org, repo, comment_id, body)

    def add_multi_pr_label(self, org: str, repo: str, number: int,
                           labels: List[str]):
        return self.pr_api.post_v5_repos_owner_repo_pulls_number_labels(
    ...
    def remove_pr_label(self, org: str, repo: str, number: int, label: str):
        return self.pr_api.delete_v5_repos_owner_repo_pulls_label(
            org, repo, number, label)

    def remove_pr_labels(self, org: str, repo: str, number: int,
                         labels: List[str]):
        return self.remove_pr_label(org, repo, number, ",".join(labels))

    def replace_pr_all_labels(self, owner, repo: str, number: int,
                              labels: List[str]):
        return self.pr_api.put_v5_repos_owner_repo_pulls_number_labels(
            owner, repo, number, labels)

    def list_pr_operation_logs(self, org: str, repo: str,
                               number: int) -> List[gitee.OperateLog]:
        return self.pr_api.get_v5_repos_owner_repo_pulls_number_operate_logs(
            org, repo, number)
    ...
    def merge_pr(self, owner, repo: str, number: int,
        ...
        self.pr_api.put_v5_repos_owner_repo_pulls_number_merge(
            owner, repo, number, body)
    ...
    def set_repo_reviewer(self, org: str, repo: str,
        ...
        return self.repository_api.put_v5_repos_owner_repo_reviewer(
            org, repo, reviewer)
    ...
    def assign_gitee_issue(self, org: str, repo: str, number: str,
        ...
        return self.issue_api.patch_v5_repos_owner_issues_number(
            org, number, body)
    ...
    def create_issue_comment(self, org: str, repo: str, number: str,
        ...
        return self.issue_api.post_v5_repos_owner_repo_issues_number_comments(
            org, repo, number, body)

    def update_issue_comment(self, org: str, repo: str, comment_id: int,
                             comment: str):
        body = gitee.IssueCommentPatchParam(body=comment)
        return self.issue_api.patch_v5_repos_owner_repo_issues_comments_id(
            org, repo, comment_id, body)

    def list_issue_comments(self, org: str, repo: str,
                            number: str) -> List[gitee.Note]:
        return self.issue_api.get_v5_repos_owner_repo_issues_number_comments(
            org, repo, number)

    def get_issue_labels(self, org: str, repo: str,
                         number: str) -> List[gitee.Label]:
        return self.label_api.get_v5_repos_owner_repo_issues_number_labels(
            org, repo, number)

    def remove_issue_label(self, org: str, repo: str, number: str, label: str):
        api = self.label_api
        return api.delete_v5_repos_owner_repo_issues_number_labels_name(
            org, repo, number, label)

    def remove_issue_labels(self, org: str, repo: str, number: str,
                            labels: List[str]):
        return self.remove_issue_label(org, repo, number, ",".join(labels))

    def add_issue_label(self, org: str, repo: str, number, label: str):
        labels = [label]
        return self.label_api.post_v5_repos_owner_repo_issues_number_labels(
            org, repo, number, labels)
    ...
    def get_issue(self, org: str, repo: str, number: str) -> gitee.Issue:
        return self.issue_api.get_v5_repos_owner_repo_issues_number(
            org, repo, number)

    def add_project_labels(self, org: str, repo: str, labels: List[str]):
        return self.label_api.post_v5_repos_owner_repo_project_labels(
            org, repo, labels)

    def update_project_labels(self, org: str, repo: str, labels: List[str]):
        return self.label_api.put_v5_repos_owner_repo_project_labels(
            org, repo, labels)

    def create_branch(self, org: str, repo: str, branch: str,
                      parent_branch: str):
        body = gitee.CreateBranchParam(branch_name=branch, refs=parent_branch)
        return self.repository_api.post_v5_repos_owner_repo_branches(
            org, repo, body)
    ...
    def set_protection_branch(self, org: str, repo: str, branch: str):
        ...
        return api.put_v5_repos_owner_repo_branches_branch_protection(
            org, repo, branch, body)

    def cancel_protection_branch(self, org: str, repo: str, branch: str):
        api = self.repository_api
        return api.delete_v5_repos_owner_repo_branches_branch_protection(
            org, repo, branch)

    def create_file(self, org: str, repo: str, branch, path, content: str,
                    commit_msg: str) -> gitee.CommitContent:
        content = base64.b64encode(content.encode()).decode()
        body = gitee.NewFileParam(
            branch=branch, content=content, message=commit_msg,
        )
        return self.repository_api.post_v5_repos_owner_repo_contents_path(
            org, repo, path, body)

    def get_path_content(self, org: str, repo: str,
                         path, ref: str) -> gitee.Content:
        return self.repository_api.get_v5_repos_owner_repo_contents_path(
            org, repo, path, ref=ref)

    def get_directory_tree(self, org: str, repo: str, sha: str,
                           recursive: int = 1) -> gitee.Tree:
        return self.git_data_api.get_v5_repos_owner_repo_git_trees_sha(
            org, repo, sha, recursive=recursive)
    ...
    def get_user_permission_of_repo(self, org: str, repo: str,
        ...
        api = self.repository_api
        return api.get_v5_repos_owner_repo_collaborators_username_permission(
            org, repo, username)

    def get_enterprise_member(self, enterprise,
                              username: str) -> gitee.EnterpriseMember:
        api = self.enterprise_api
        return api.get_v5_enterprises_enterprise_members_username(
            enterprise, username)
